...
open class SOContainerViewController: UIViewController, UIGestureRecognizerDelegate {
     ...
     */
    open var widthForPanGestureRecognizer: Int = 20
        ...
        set {
            ...
            if let vc = _topViewController {
                ...
                if widthForPanGestureRecognizer > 0 {
                    print("widthForPanGestureRecognizer0 = \(widthForPanGestureRecognizer)")
                    ...
                    vc.view.addSubview(panView)
                     ...
                     print("widthForPanGestureRecognizer1 = \(widthForPanGestureRecognizer)")
    ...
    open override func viewDidLoad() {
        ...
        case .phone:
            ...
            print("iPhone and iPod touch style UI = \(self.sideMenuWidth)")
        case .tv:
            //  frame.size.width = self.sideMenuWidth
            print("tvOS style UI")
        default:
    ...
    fileprivate func viewPulledOutMoreThanHalfOfItsWidth(_ viewController: UIViewController) -> Bool {
        ...
        if menuSide == .left{
            print("""
                ...
                fabs(frame.origin.x) = \(fabs(frame.origin.x)),
                
                frame.size.width = \(frame.size.width),
                
                frame.size.width / 2 = \(frame.size.width / 2),
                
                fabs(frame.origin.x) < frame.size.width / 2 = \(fabs(frame.origin.x) < frame.size.width / 2)
    ...
    @IBAction internal func moveMenu(_ panGesture: UIPanGestureRecognizer) {
     ...
     //   print("initialCenter😍 = \(initialCenter)")
        ...
        print("""
            ...
            PANGESTER FOR X = \(String(describing: panGesture.view?.center.x))
            
            """)
        
        if panGesture.state == UIGestureRecognizerState.changed {
           //  print("PanGester.State1 = \( panGesture.state)")
            if let sidebarView = self.sideViewController?.view {
                print("TransPointIN = \(translatedPoint)")
          ...
          //  print("PanGester.State2 = \( panGesture.state)")
            ...
            let newCenter = CGPoint(x: initialCenter.x + translatedPoint.x, y: initialCenter.y + translatedPoint.y)
            
             panGesture.view?.center = newCenter
             // print("PanGester.State3 = \( panGesture.state)")
        } else if panGesture.state == .began {
             // print("PanGester.State3 = \( panGesture.state)")
            
//            let newCenter = CGPoint(x: 0, y: 0)
//           // piece.center = newCenter
           //panGesture.view?.center = initialCenter
        }
    }
